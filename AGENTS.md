...
- **Build/start memory**: Both `pnpm run build` and `pnpm start` may crash with "Aborted (core dumped)" under default Node.js memory limits. In the Cloud Agent VM, `--max-old-space-size=4096` is not enough; use `NODE_OPTIONS="--max-old-space-size=8192"` for reliable compilation.
